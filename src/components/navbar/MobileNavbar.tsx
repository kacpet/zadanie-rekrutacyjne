...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full font-inter font-[14px]">
      ...
      <div
        className={`absolute top-full z-40 bg-white shadow-lg transition-all duration-300 ease-out origin-top px-7 hidden lg:flex flex-col ${
          loup ? "right-155" : "right-109"
        } ${open ? "translate-y-0" : "-translate-y-[110%]"}`}
      >
        <GradientLink href="#offer" className={menuLinkClass}>
          oferta 1
        </GradientLink>
        ...
        <GradientLink href="#offer" className={`${menuLinkClass} mb-2`}>
          oferta 4
        </GradientLink>
      </div>
</nav>
